...
class ObjectNatCalculator:
    ...
    def _calculate_absolute(
        ...
        result = (supplied_demand_after - supplied_demand_before).apply(
            lambda x: max(0, x)
        ) - (unsupplied_demand_after - unsupplied_demand_before).apply(
            lambda x: max(0, x)
        )
        return result
    ...
    def _calculate_effects(
        ...
        total_us_demands_before = effects["us_demands_without_before"].fillna(0)
        total_us_demands_after = effects["us_demands_without_after"].fillna(0)
        ...
        project_total_us_demands_after = effects[effects["is_project"]][
        ...
        effects["absolute_scenario_project"] = None
        effects.loc[effects["is_project"], ["absolute_scenario_project"]] = (
            self._calculate_absolute(
        ...
        effects["index_scenario_project"] = None
        effects.loc[effects["is_project"], ["index_scenario_project"]] = (
    ...
    def estimate_effects(
        ...
        effects = provision_after.merge(
            provision_before, how="outer", on=["building_id"]
        )
        effects["geometry"] = effects.apply(
            lambda x: (
                x["geometry_x"] if not pd.isna(x["geometry_x"]) else x["geometry_y"]
            ),
            axis=1,
        )
        effects.drop(columns=["geometry_x", "geometry_y"], inplace=True)
        effects["demand"] = effects["demand_x"].fillna(0) + effects["demand_y"].fillna(
            0
        )
        ...
        effects = gpd.GeoDataFrame(
            effects, geometry="geometry", crs=provision_before.crs
        )
        return effects
